...
Notes:
- The built-in MINI_DB is intentionally tiny. Replace/extend it with your real network.
- Units:
    * Concentrations: mol/m^3
    * Gas species state variable: mol in headspace
    * k (bimolecular): m^3/(mol*s); first-order: s^-1
    * Henry: kH in mol/(m^3*Pa), p in Pa
    * Dose rate: J/(m^3*s)
...
class GasExchange:
    """Linear mass transfer with Henry’s law:
       flux [mol/m^3/s] = kLa * (C_liq - kH * p_gas)
...
class System:
    ...
    sources: Dict[str, float]                 # species -> mol/m^3/s source into LIQUID pool
    T: float                                  # K
    V_liq_m3: float
    V_gas_m3: float
...
def build_sources_from_G(g_values: Dict[str, float], dose_rate_J_m3_s: float) -> Dict[str, float]:
    """S_i [mol/m^3/s] = G_i [molecules/100eV] * dose_rate / (100 eV in J) / N_A"""
...
MINI_DB = {
    "kernels": {
        "chloride": {
            "species": ["Cl-", "Cl2•-", "Cl2_diss", "e_s-"],   # add/adjust as needed
            "phases":  {"Cl-": "liq", "Cl2•-": "liq", "Cl2_diss": "liq", "e_s-": "liq"},
            "reactions": [
                ...
                {"name": "e + Cl2_diss -> Cl2•-",
                 ...
                 "products":  {"Cl2•-": 1.0},
                 "reversible": False,
                 "params": {"k_ref": 1.0e-3}},   # m^3/(mol*s)  (placeholder)
            ],
        },
        "fluoride": {
    ...
    "henry": {  # placeholder Henry constants
        "Cl2_mol_m3_Pa": 2.0e-5,   # mol/(m^3*Pa)
        "F2_mol_m3_Pa":  1.0e-5,
    },
    ...
    "G_values": {
        "gamma": {
            "chloride": {"Cl2_diss": 0.001, "e_s-": 0.001},   # molecules/100eV (placeholders)
...
def build_system(config: Dict, db: Optional[Dict] = None) -> System:
    ...
    Required/recognized keys (matching your README/builder):
      ...
      initial_concentrations: {species -> mol/m^3}
      metals: {MetalName -> {species -> mol/m^3}}
...
def _rhs(system: System, t: float, y: np.ndarray) -> np.ndarray:
    ...
    if system.gas_exchanges and system.V_gas_m3 > 0.0:
        for gx in system.gas_exchanges:
            ...
            flux = gx.kLa_s * (C_liq - C_eq)  # mol/m^3/s (positive = to gas)
...
# ---- integration
def integrate_system(system: System, t_final: float, n_steps: int = 200,
    ...
    if _HAVE_SCIPY and method == "auto":
        sol = solve_ivp(lambda tt, yy: _rhs(system, tt, yy), (t0, t[-1]), y0,
                        t_eval=t, method="Radau", vectorized=False, atol=1e-10, rtol=1e-6)
        Y = sol.y.T
...
# ---- plotting
def quick_plot(t: np.ndarray, C: np.ndarray, system: System, species_to_plot: Optional[List[str]] = None):
    if not _HAVE_MPL:
    ...
    for s in targets:
        i = sp_idx[s]
        y = C[:, i]
        label = f"{s} ({'mol/m^3' if system.species[i].phase=='liq' else 'mol'})"
        plt.semilogy(t, y, label=label)
    plt.xlabel("time [s]"); plt.legend(); plt.tight_layout(); plt.savefig('youplot.png'); plt.show()
...
def main(argv=None):
    ...
    if not argv:
        ...
        demo = {
            ...
            "initial_concentrations": {"Cl-": 5000.0, "e_s-": 1e-6},
            "gas_species": ["Cl2"]
        }
        system = build_system(demo)
        t, C, extra = integrate_system(system, t_final=5.0, n_steps=200)
        print("Species:", extra["species"])
        print("Final state:", C[-1])
        quick_plot(t, C, system, species_to_plot=["Cl2•-", "Cl2_diss"])
        return

    cfg = _load_config(argv[0])
    t_final = float(argv[1]) if len(argv) > 1 else 5.0
    n_steps = int(argv[2]) if len(argv) > 2 else 200
